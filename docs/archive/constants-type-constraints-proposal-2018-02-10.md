...
class Foo {}.new;
constant @foo = Foo.new;          # List.new(Foo.new)
constant @foo = Foo.new,;         # List.new(Foo.new)
constant @foo = Foo.new, Foo.new; # List.new(Foo.new, Foo.new)
...
constant %foo  = :{:42foo, :70bar}; # Hash[Mu,Any].new((:42foo, :70bar))
...
my $bar        := class Bar does Positional {}.new;
my $bar-scalar  = class Bar does Positional {}.new;
constant @foo = $bar;        # Bar.new
constant @foo = $bar-scalar; # List.new(Bar.new)
constant @foo = $bar,;       # List.new(Bar.new)
constant @foo = $bar, $bar;  # List.new(Bar.new, Bar.new)
```


## Deviations from Current Behaviour

I'm dubious there are any spec tests actually covering it, but if any 6.c
language spectests will get broken, the proposal then is to implement the
behaviour in 6.d language.


```perl6
class Bar does Positional {}.new;
BEGIN my $bar-scalar = Bar.new;
constant @foo = Bar.new;  # PROPOSED: Bar.new
                          # CURRENT:  List.new(Bar.new)
```
...
```perl6
# Int constant, value 42
       constant foo  = 42;
my Int constant foo  = 42;
my Int constant foo  .=new: 42;
```

```perl6
# Not-so-constant Array[Numeric].new(1, 2, 3)
my Array[Numeric] foo .= new: 1, 2, 3;
foo.push: 42; # it's an Array, so this works
```
