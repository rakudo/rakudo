...
2.  a set of action methods to transform the parse tree into
    an abstract syntax tree (src/parser/actions.pm)
3.  the main compiler object (perl6.pir)
4.  builtin functions and runtime support (src/builtins/,
    src/classes/, src/pmc/)
...
(captured into $<sym>), followed by an expression and then a block.
