...
attributes added to the package. Returns the the type object that we're
going to actually install into the namespace or lexical pad, or just return if
it's an anonymous declaration.
...
=item new($name, :$has-accessor, :$rw, :$handles)
...
acomposition-time issue and not a runtime one.
