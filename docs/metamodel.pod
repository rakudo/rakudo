...
There are three important things. First, the actual composer - implementing the
...
a prototype based one then you'd return your most basic prototype as the HOW.
...
to the composition list. Certainly all of the built-in implementations of this
do not actually perform any composition at this point. Instead, they add the
composable to a "to do" list, and at the point we call "compose" to finish the
composition of the package, and the application of all the composables takes
place. You probably want to do something similar.
...
calling the composition hook on all attributes added to the package.
...
=item compose($attr, $meta-package)

Takes the attribute and does any final composition tasks (such as installing
any accessor methods and/or delegators). The second parameter is the
meta-object of the package that the attribute belongs to; you can call
.^add_method on it to add methods, for example.
...
the model is intended for more general compositiony things rather than just
...
from something that has a different metaclass. For now, we forbid this unless
the metaclass of the thing we're inheriting from is a superclass of the thing
that is inheriting. This means that all other types of class-ish things that
want to have a custom metaclass should subclass ClassHOW (either directly or
transitively). Thus, this is fine:
...
the problem of the behaviors of two 
...
design of the metamodel.
    ...
    class AspectHOW {
        ...
        method add_method($meta, $name, $method) {
            $meta.method_wrappers{$name} = $method;
        }
        
        multi method composer_for($meta, $ where { .can('methods') }) {
            return AspectComposer;
        }
        
        multi method composer_for($meta, Object) {
            die "Can only apply aspects to things that expose methods";
        }

        method compose($meta) {
            # We'll just install ourself into the namespace. XXX Or should we
            # have some standard way of making a "type object"?
            return $meta;
        }
...
Rakudo has a compile-time representation of a package that it is currently in
the process of compiling. This is the thing that ends up actually generating
...
for - those use a subclass of it, such as Perl6::Compiler::Role. You may modify
%*PKGCOMPILER, again keying on $*PKGDECL, to specify something other than the
default. You then should write a subclass of an existing handler for this and
