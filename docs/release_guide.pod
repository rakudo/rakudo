...
actual values using C<git describe> in appropriate repo and edit the MOAR_REVISION
...
has to be added to the top of C<docs/ChangeLog>. Do not include the "IGNORE"
section from the wiki. These are used later in the commit message.
...
Review the 6.c, 6.d errata branches of Raku/roast - you do not
want to find out about failures on the day of the release, in
case they are indicators of an actual issue, and not just that
the test file needs to be updated.
...
As the actual release date nears, use the C<releasable6>
...
Note: When copying the changelog into the announcement - remove the commit ids.
These don't belong in the release info. Feel free to re-word and combine entries
in the changelog, especially in the copy in the release announcement - they don't
have to match line for line.
...
e.g. https://github.com/rakudo/rakudo/commit/8545a055) and push the release branch up.
This has to be done before the ake commands run as they update the remote.
Be sure to include the deliberately ignored commits here for the C<releasable> bot.
...
the directory to do a release. All the steps are setup as individual
C<ake> steps.
...
- Branch to get Rakudo from is, e.g.: C<release-2021.10>
...
behavior.
  ...
  TEST_JOBS=24 VERSION=2021.10 VERSION_MOAR=2021.10 BRANCH_ROAST=master BRANCH_RAKUDO=release-2021.10 BRANCH_NQP=main ake rakudo-stress-errata-6c

  TEST_JOBS=24 VERSION=2021.10 VERSION_MOAR=2021.10 BRANCH_ROAST=master BRANCH_RAKUDO=release-2021.10 BRANCH_NQP=main ake rakudo-stress-errata-6d
...
Once this is done, create the artifact...
...
... And test it afterwards. You can manually verify the build worked and shows the correct version with:

    ./temp/rakudo-test/rakudo-$VERSION/install/bin/raku --version

Now you can test the built version. Note that this deletes the directory, so after this step you will be
unable to rerun the previous command.
...
This step also tries to upload tarballs, but this fails. You can run this step manually:
    ...
    scp -P 222 nqp/nqp-2021.10.tar.gz*  rakudo.org@trinity.raku.org:/var/www/htdocs/rakudo.org/binaries/nqp/
    scp -P 222 rakudo/rakudo-2021.10.tar.gz*  rakudo.org@trinity.raku.org:/var/www/htdocs/rakudo.org/binaries/rakudo/

Uploading requires ssh access to a file server.  If you require access, open an issue on the
L<infra repository|https://github.com/raku/infra>.

=item *

Create PR and merge on remote (github) because ake puts commits there (that
wouldn't be in your local). When merging, do NOT squash the commit.

=item *

Use the C<./github-release> script to create both NQP and Rakudo Github releases.
Refer to MoarVM release guide for the details about artifacts attachment. Assuming
you have an appropriate github token in the given file, you can update the sample
release # here and run:

    ./github-release --asset=rakudo/rakudo-2021.10.tar.gz --asset=rakudo/rakudo-2021.10.tar.gz.asc --repo=rakudo/rakudo --tag=2021.10 --token=$(cat ~/.ghtoken-raku-release) --announcement=rakudo/docs/announce/2025.10.md
    ./github-release --asset=nqp/nqp-2021.10.tar.gz --asset=nqp/nqp-2021.10.tar.gz.asc --repo=raku/nqp --tag=2021.10 --token=$(cat ~/.ghtoken-raku-release)
...
Previous releases were bundled as part of monthly Parrot releases.
  ...
  2025-08-28   Rakudo #185 "2025.08"           (jdv)
  2025-10-18   Rakudo #186 "2025.10"           (Coke)
  2025-11-15   Rakudo #187 "2025.11"           (Coke)
  2025-12-20   Rakudo #188 "2025.12"           (Coke)
  2026-01-24   Rakudo #189 "2026.01"           (jdv)
  2026-02-28   Rakudo #190 "2026.02"           (jdv)
  2026-03-28   Rakudo #191 "2026.03"           (Coke)
  2026-04-25   Rakudo #192 "2026.04"           (Coke)
  2026-05-23   Rakudo #193 "2026.05"           (Coke)
  2026-06-27   Rakudo #194 "2026.06"           (Coke)
  2026-07-24   Rakudo #195 "2026.07"           (Coke)
  2026-08-21   Rakudo #196 "2026.08"           (Coke)

=head1 COPYRIGHT

Copyright © 2009-2026, The Raku Foundation.
