...
some releases to ensure any compatibility issues are resolved.
  ...
  2025-01-25   Rakudo #179 (Coke)
  2025-02-22   Rakudo #180 (Coke|jdv)
...
Update Rakudo’s leap-second tables; Review the list at
L<https://www.nist.gov/pml/time-and-frequency-division/time-realization/leap-seconds>
and compare against F<src/core.c/Rakudo/Internals.rakumod>

  raku tools/add-leap-second.raku <the-date> [<the-source>]

This version of the script requires the release manager to B<manually> check
to see if a new leap second has been announced, and then call this script
if needed.

If updated, F<src/core.c/Rakudo/Internals.rakumod>
...
version beforehand.
  ...
  (check version aka raku --version)
...
the script to automate this part. Uploading requires ssh access to a file server.
If you require access, open an issue on the [infra repository](https://github.com/raku/infra).
...
Note that the artifacts upload is broken so you have to do it manually.

Copy the announce into the github release description field.
...
Previous releases were bundled as part of monthly Parrot releases.
  ...
  2024-01-25   Rakudo #179 "2025.01"           (Coke)
