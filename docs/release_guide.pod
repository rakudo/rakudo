...
on the release year and month.  Historically, releases had code names based on
an active Perl Mongers group, reflecting Raku's original "Perl 6" status.

See the list of all previous releases at the end of this document.
...
Currently the release is only known to work correctly on linux.

=item *

Ensure you have a working Raku with C<Ake> module installed .
...
A working install of L<Blin|https://github.com/Raku/Blin>, or someone else
to run it for you.
...
First, bump MoarVM and NQP if necessary. You can use
C<git describe> in appropriate repo to get the target value, and edit the MOAR_REVISION
...
post it as a new Rakudo blocker ticket. You may have to remove the
previous/next output due to size constraints on github issues.
...
Currently the C<Red> module is unable to be tested by Blin - please
build and test this with Rakudo HEAD close to release day to verify
no issues.

=item *

During the month leading up to the release, it's a good idea to keep
the Changelog up to date. Use the C<releasable6>
IRC bot to prepare a ChangeLog. The bot resides in
the C<#raku-dev> channel, call it as C<releasable6: status>
to obtain a report with commits that haven't been logged yet

Do not commit directly to Rakudo, but instead modify the 
L<ChangeLog wiki|https://github.com/rakudo/rakudo/wiki/ChangeLog-Draft>

Ensure you have a proper list of "ignored" commits, such as
merge commits, tiny ones, reverts etc. While the bot will give
you the primary commit message, it's also OK to combine commits
into a single Changelog entry, esp. for projects like RakuAST.
...
Remind people of the upcoming release, invite people to update
L<ChangeLog|https://github.com/rakudo/rakudo/wiki/ChangeLog-Draft> with any
updates the bot shows as missing. At this point, no changes to the rakudo's
copy in C<docs/ChangeLog>
...
Go to this directory to do a release:

    cd rakudo/tools/releasable

This guide will step you through all the individual steps supported
by C<Ake>, and note when manual effort is required.
...
Prepare environment variables for the invocation, for example:
    ...
    TEST_JOBS=24 VERSION=2021.10 VERSION_MOAR=2021.10 BRANCH_ROAST=master BRANCH_RAKUDO=release-2021.10 BRANCH_NQP=main ake foo
...
affects what branches will be used to push commits to. Use these variables
consistently whenever invoking C<ake> below: we show C<ENV=...>
instead of repeating the full list each time.

=back

=item 6.

Execute Akefile stages one by one, with appropriate env variables set.
...
so it recommended to run stages separately:
    ...
    ENV=... ake rakudo-build
    ENV=... ake rakudo-test
    ENV=... ake rakudo-inline-perl5
    ENV=... ake rakudo-stress
...
behavior.
    ...
    ENV=... ake rakudo-stress-errata-c
    ENV=... ake rakudo-stress-errata-d
...
version beforehand.
    ...
    ENV=... ake rakudo-archive
    ENV=... ake rakudo-extract
    ENV=... ake rakudo-post-build
    (check version aka ./temp/rakudo-test/$rel-branch-name/install/bin/raku --version)
    ENV=... ake rakudo-post-test
...
release fails to create for some reason, you can create it by hand on github. Note - this seems
to always fail, not just with the file upload.
