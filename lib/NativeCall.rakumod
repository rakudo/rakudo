...
# Provide scoping, to make sure EXPORT is excluded from this scope
our module NativeCall {

#- re-export NativeCall::Types constants ---------------------------------------
  ...
  $libname where Str | Callable | List | IO::Path | Distribution::Resource
    ...
    method setup-nativecall() {
        ...
        unless $use-dispatcher {
            ...
            # finish compilation of the original routine so our changes
            # won't become undone right afterwards
            ...
            my $replacement := -> |c {
                ...
                my Mu $args :=
                  nqp::getattr(nqp::decont(c), Capture, '@!list');
                ...
                if $!any-callbacks {
                    ...
                    while $i < $nr-args {
                        ...
                        nqp::bindpos(
                          $args, $i, nqp::getattr($arg,Code,'$!do')
...
our proto guess_library_name(|) is export(:TEST) {*}
...
multi sub nativecast(Int $target-type, $source) {
    nqp::nativecallcast(
      nqp::decont($target-type), Int, nqp::decont($source)
    )
}
multi sub nativecast(Num $target-type, $source) {
    nqp::nativecallcast(
      nqp::decont($target-type), Num, nqp::decont($source)
    )
}
multi sub nativecast($target-type, $source) {
    nqp::nativecallcast(
...
our sub check_routine_sanity(Routine $routine) is export(:TEST) {
...
sub EXPORT(|) {
    ...
    my $native_trait := multi trait_mod:<is>(Routine $r, :$native!) {
        NativeCall::check_routine_sanity($r);
