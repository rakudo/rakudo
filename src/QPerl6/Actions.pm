...
class QPerl6::Actions is HLL::Actions {
    ...
    method pod_block:sym<delimited>($/) {
        make QPerl6::Pod::any_block($/);
    }

    method pod_block:sym<delimited_raw>($/) {
        make QPerl6::Pod::raw_block($/);
    }

    method pod_block:sym<delimited_table>($/) {
        make QPerl6::Pod::table($/);
    }
    ...
    method pod_content:sym<text>($/) {
        ...
        my $past := QPerl6::Pod::serialize_array(@ret);
        make $past<compile_time_value>;
    }

    method pod_textcontent:sym<regular>($/) {
        my @t     := QPerl6::Pod::merge_twines($<pod_string>);
        my $twine := QPerl6::Pod::serialize_array(@t)<compile_time_value>;
        make QPerl6::Pod::serialize_object(
    ...
    method pod_textcontent:sym<code>($/) {
        ...
        my $t := subst($<text>.Str, /\n$s/, "\n", :global);
        $t    := subst($t, /\n$/, ''); # chomp!
        my $past := QPerl6::Pod::serialize_object(
            'Pod::Block::Code',
            :content(QPerl6::Pod::serialize_aos([$t])<compile_time_value>),
        );
        make $past<compile_time_value>;
    }
    ...
    method pod_formatting_code($/) {
        if ~$<code> eq 'V' {
            ...
            my @t    := QPerl6::Pod::build_pod_string(@content);
            my $past := QPerl6::Pod::serialize_object(
                'Pod::FormattingCode',
                :type(
                ...
                :content(
                    QPerl6::Pod::serialize_array(@t)<compile_time_value>
                )
            );
            make $past<compile_time_value>;
        }
    ...
    method pod_string($/) {
        ...
        make QPerl6::Pod::build_pod_string(@content);
    }
...
#            }
    ...
    sub regex_coderef($/, $code, $qast, $scope, $name, @params, $block, $traits?, :$proto, :$use_outer_match) {
        ...
        $past[0].push(PAST::Var.new(
            ...
            :viviself(QAST::Var.new( :name('self'), :scope('lexical') ))));
        ...
        for @values {
            ...
            if $_.returns() eq 'Pair' {
                ...
                if $_[2]<has_compile_time_value> {
                    ...
                    my $ok;
                    try {
                        $cur_value := QPerl6::ConstantFolder.fold(
                                        $_[2], $*W.cur_lexpad(), $*W
                                    )<compile_time_value>;
                        $ok := 1;
                    }
...
class QPerl6::RegexActions is QRegex::P6Regex::Actions {
