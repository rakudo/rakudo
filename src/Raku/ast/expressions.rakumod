  ...
  is RakuAST::CheckTime
{
    ...
    method IMPL-TO-QAST(RakuAST::IMPL::QASTContext $context, *%opts) {
        if $!thunks {
            ...
            # Cache the result of thunk value QAST generation
            $!thunks.IMPL-THUNK-VALUE-QAST($context)
        }
        else {
            # Directly call IMPL-EXPR-QAST with the provided options
  ...
  is RakuAST::CheckTime
{
    ...
    method IMPL-CURRIES() {
        ...
        # Fast path: Use direct comparison for common operators
    ...
    method IMPL-INFIX-QAST(
      RakuAST::IMPL::QASTContext $context,
        ...
        # Fast path: Provide direct mapping for most commonly used operators to avoid hash lookups
        my str $op = self.IMPL-GET-DIRECT-QAST-OP($!operator);
        if $op {
        ...
        # Determine call type
        my str $call-op = self.properties.chain ?? 'chain' !! 'call';
    ...
    # Helper method: Provide fast path mapping for common operators
    method IMPL-GET-DIRECT-QAST-OP(str $operator) {
        # Direct comparison instead of hash lookup for better performance
        if $operator eq '||' || $operator eq 'or' {
            return 'unless';
        } elsif $operator eq '&&' || $operator eq 'and' {
            return 'if';
        } elsif $operator eq '^^' || $operator eq 'xor' {
            return 'xor';
        } elsif $operator eq '//' {
            return 'defor';
        }
        return '';  # Indicates no direct mapping
    }
    ...
    method IMPL-INTERPRET(RakuAST::IMPL::InterpContext $ctx, List $operands) {
        # Fast path: Handle special cases for common operators
        my $op := $!operator;
        
        # Quick handling for comma operator (during core setting compilation)
        if $op eq ',' && $*COMPILING_CORE_SETTING {
            # Create List directly without calling resolve for performance optimization
            my @values;
            my Mu $list_items := self.IMPL-UNWRAP-LIST($operands);
            my int $count := nqp::elems($list_items);
            
            for 0..^$count -> int $i {
                nqp::push(@values, nqp::atpos($list_items, $i).IMPL-INTERPRET($ctx));
            }
            
            my $list := nqp::create(List);
            nqp::bindattr($list, List, '$!reified', @values);
            return $list;
        }
        
        # Normal path: Check if resolved and process
        if self.is-resolved && nqp::istype(self.resolution, RakuAST::CompileTimeValue) {
            # Optimized operand handling
            my Mu $list_items := self.IMPL-UNWRAP-LIST($operands);
            my int $count := nqp::elems($list_items);
            my @values;
            ...
            # Call compile-time value operator
            return self.resolution.compile-time-value(|@values);
        }
        
        # Default case: return Nil if cannot be interpreted
        Nil
    }
  ...
  is RakuAST::Infix
{
    ...
    method IMPL-INFIX-QAST(
      RakuAST::IMPL::QASTContext $context,
        ...
        # Optimized implementation: Build QAST node for mixin operator
        my $qast := QAST::Op.new(
            :op('call'),
        ...
        # Fast path handling: Different processing based on right operand type
        if nqp::istype($right-qast, QAST::Op) && $right-qast.op eq 'call' {
            # Case 1: Handle function call as right operand
            if $right-qast.name && +@($right-qast) == 1 {
                $qast.push($right-qast);
            }
            else {
                # Case 2: Check for compile-time value parameters
            ...
            # Case 3: Handle list operator
            for @($right-qast[0]) {
                $qast.push($_);
            }
        }
        else {
            # Default case: Add right operand directly
            $qast.push($right-qast);
        }
        
        # Return the built QAST node
    }
}
  ...
  is RakuAST::CheckTime
{
    ...
    method is-pure() {
        # Fast path: directly compare common impure operators to avoid hash lookups
        if $!operator eq '--' || $!operator eq '++' || $!operator eq '--⚛' || $!operator eq '++⚛' {
            return False;
        }
        # Default is a pure operator
        return True;
    }
    ...
    method IMPL-PREFIX-QAST(RakuAST::IMPL::QASTContext $context, Mu $operand-qast) {
        # Fast path: Direct handling for the most common prefix operators
        my str $direct-qast-op = self.IMPL-GET-DIRECT-PREFIX-QAST-OP($!operator);
        if $direct-qast-op {
            my $op := QAST::Op.new( :op($direct-qast-op), $operand-qast );
            self.IMPL-ADD-COLONPAIRS-TO-OP($context, $op);
            return $op;
        }
        
        # Handle other prefix operators
        my $name;
        if self.is-resolved || !$*COMPILING_CORE_SETTING {
            $name := self.resolution.lexical-name;
        }
    ...
    # Helper method: Check if there's a direct QAST opcode mapping
    method IMPL-GET-DIRECT-PREFIX-QAST-OP(str $operator) {
        # For prefix operators that can be directly mapped to QAST opcodes, provide a fast path
        # Note: This needs to be adjusted according to the actual implementation, this is an example
        return '';  # No direct mapping by default
  ...
  is RakuAST::WhateverApplicable
{
    ...
    method IMPL-EXPR-QAST(RakuAST::IMPL::QASTContext $context) {
        # Precompute QAST representation for operand
        my $operand-qast := $!operand.IMPL-TO-QAST($context);
        
        # Fast path handling for common operators
        if nqp::istype($!prefix, RakuAST::Prefix) {
            my str $op = $!prefix.operator;
            # Can add special handling for specific high-performance prefix operators
            # For example: simple arithmetic, logical operations, etc.
        }
        
        # Call the prefix operator's QAST generation method
        return $!prefix.IMPL-PREFIX-QAST($context, $operand-qast);
    }
    ...
    method propagate-sink(Bool $is-sunk) {
        # Prefix operators never propagate sink context to operands
        # This is to prevent side-effect operators from being optimized away in sink context
  ...
  is RakuAST::ParseTime
{
    ...
    method PERFORM-CHECK(RakuAST::Resolver $resolver, RakuAST::IMPL::QASTContext $context) {
        # Second chance to resolve to avoid bootstrapping issues in the setting
        unless self.is-resolved {
            my $resolved := $resolver.resolve-lexical(
                self.is-multislice
                    ?? '&postcircumfix:<[; ]>'
