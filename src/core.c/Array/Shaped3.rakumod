my role Array::Shaped3 does Array::Shaped is implementation-detail {
    ...
    multi method AT-POS(::?CLASS:D:
      Int:D $one, Int:D $two, Int:D $three
    ) is raw {
        nqp::ifnull(
          nqp::atpos3d(nqp::getattr(self,List,'$!reified'),$one,$two,$three),
          AT-POS-CONTAINER(self, $one, $two, $three)
    ...
    multi method ASSIGN-POS(::?CLASS:D:
      Int:D $one, Int:D $two, Int:D $three, \value
    ) {
        my \reified := nqp::getattr(self,List,'$!reified');
        nqp::ifnull(
          nqp::atpos3d(reified,$one,$two,$three),
