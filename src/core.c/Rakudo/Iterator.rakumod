...
# Methods that generate an Iterator (in alphabetical order)
    ...
    my class AntiPair does Iterator {
        ...
        method push-all(\target --> IterationEnd) {
            ...
            nqp::until(
              nqp::eqaddr((my \pulled := $!iter.pull-one),IterationEnd),
              target.push(Pair.new(pulled,+(++$key)))
            )
        }
    ...
    my class Combinations does PredictiveIterator {
        ...
        method pull-one() {
            ...
            while $running && nqp::elems($stack) -> int $elems {
                $index = $elems - 1;
                $value = nqp::pop_i($stack);
...
#?endif
              ...
              nqp::stmts(  # doesn't sink
                ...
                nqp::while(  # doesn't sink
                  ...
                  nqp::if(
                    ...
                    last   #  XXX this probably doesn't do what is intended
                  ),
                  $!push-target.push($value)
                ),
                ($!wanted = $!wanted - $i)
    ...
    my class IntRange does PredictiveIterator {
        ...
        method skip-one() { nqp::isle_i(++$!i,$!last) }
        method push-exactly(\target, int $batch-size) {
    ...
    # Class for iterating native int arrays and nqp::list_u
    my class native_u does PredictiveIterator {
        has $!list;
        has int $!i;
        ...
        method push-all(\target --> IterationEnd) {
            ...
            nqp::while(
              nqp::islt_i(++$i,nqp::elems($list)),
              target.push(nqp::atposref_u($list,$i))
            );
            $!i = $i;
        }
    ...
    method native_u(Mu \list --> Iterator:D) { native_u.new(list) }
    ...
    my class Pairs does Iterator {
        ...
        method push-all(\target --> IterationEnd) {
            ...
            nqp::until(
              nqp::eqaddr(($pulled := $!iter.pull-one),IterationEnd),
              target.push(Pair.new(++$key,$pulled))
            )
        }
        method is-lazy() { $!iter.is-lazy }
        method is-deterministic(--> Bool:D) { $!iter.is-deterministic }
    ...
    my class ReifiedReverseIterator does PredictiveIterator {
        ...
        method skip-one() { nqp::isge_i(--$!i,0) }
