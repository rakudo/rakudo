...
my class Str does Stringy { # declared in BOOTSTRAP
    ...
    my &POST-MATCH  := Match.^lookup("MATCH");  # vivify object
    my &POST-STR    := Match.^lookup("STR");    # stringify object
    ...
    method !APPLY-MATCHES(\matches,$replacement,\cds,\SDS,\word_by_word,\space,\case,\mark) {
        ...
        if SDS || space || case || mark || callable {
            ...
            for flat matches -> $m {
                ...
                nqp::push_s(
                  $result,
                  nqp::substr($str,$prev,nqp::unbox_i($m.from) - $prev)
                );
