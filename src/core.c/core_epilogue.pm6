...

{
    ...
    Metamodel::Configuration.set_language_revision_role(
        my role LanguageRevision {
            has Str $!p6rev;
            ...
            multi method Int(::?CLASS:D:)     { nqp::box_i(nqp::unbox_i(self), Int) }
            multi method Numeric(::?CLASS:D:) { self.Int }
            multi method Real(::?CLASS:D:)    { self.Int }
            method Version(::?CLASS:D:) {
                nqp::getcomp('Raku').lvs.as-public-repr(nqp::unbox_i(self), :as-version)
            }
        }
    );
    BEGIN CORE-SETTING-REV.^mixin: LanguageRevision;
}
