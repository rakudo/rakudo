class CompUnit::Repository::Installation does CompUnit::Repository::Locally does CompUnit::Repository::Installable {
    ...
    my class InstalledDistribution is Distribution::Hash {
        method content($address) {
            ...
            my $file = $entry
                ?? $.prefix.concat-with('sources').concat-with($entry{$address}<file>)
                !! $.prefix.concat-with('resources').concat-with($.meta<files>{$address});
...
sub MAIN(:$name is copy, :$auth, :$ver, *@, *%) {
    ...
    method !sources-dir() {
        my $sources = $.prefix.concat-with('sources');
        $sources.mkdir unless $sources.e;
        $sources
    }

    method !resources-dir() {
        my $resources = $.prefix.concat-with('resources');
        $resources.mkdir unless $resources.e;
        $resources
    }

    method !dist-dir() {
        my $dist = $.prefix.concat-with('dist');
        $dist.mkdir unless $dist.e;
        $dist
    }

    method !bin-dir() {
        my $bin = $.prefix.concat-with('bin');
        $bin.mkdir unless $bin.e;
        $bin
    }

    method !add-short-name($name, $dist, $source?) {
        my $short-dir = $.prefix.concat-with('short');
        my $id = nqp::sha1($name);
        my $lookup = $short-dir.concat-with($id);
        $lookup.mkdir;
        $lookup.concat-with($dist.id).spurt(
                "{$dist.meta<ver>  // ''}\n"
            ~   "{$dist.meta<auth> // ''}\n"
            ~   "{$dist.meta<api>  // ''}\n"
            ~   "{$source // ''}\n"
    ...
    method !read-dist($id) {
        my $dist = Rakudo::Internals::JSON.from-json($.prefix.concat-with('dist').concat-with($id).slurp);
    ...
    method !repository-version(--> Int:D) {
        ...
        my $version-file = $.prefix.concat-with('version');
    ...
    method upgrade-repository() {
        ...
        my $precomp-dir = $.prefix.concat-with('precomp');
        ...
        if ($version < 1) {
            for $short-dir.dir -> $file {
                ...
                for @ids -> $id {
                    ...
                    $file.concat-with($id).spurt("{$dist<ver> // ''}\n{$dist<auth> // ''}\n{$dist<api> // ''}\n");
                }
            }
        }
        if ($version < 2) {
            for $dist-dir.dir -> $dist-file {
                ...
                for eager $files.keys -> $file {
                    $files{"resources/$file"} = $files{$file}:delete
                        if $resources-dir.concat-with($files{$file}).e
                        and not $.prefix.concat-with($file).e; # bin/ is already included in the path
                }
                $dist-file.spurt: Rakudo::Internals::JSON.to-json(%meta);
            }
        }
        $.prefix.concat-with('version').spurt('2');
        $!version = 2;
    }
    ...
    multi method install(Distribution $distribution, Bool :$force) {
        ...
        my $dist-id = $dist.id;
        my $dist-dir = self!dist-dir;
        if not $force and $dist-dir.concat-with($dist-id) ~~ :e {
            $lock.unlock;
        ...
        for $dist.meta<provides>.kv -> $name, $file is copy {
            ...
            my $destination = $sources-dir.concat-with($id);
        ...
        for %files.kv -> $name-path, $file is copy {
            ...
            my $destination = $resources-dir.concat-with($id); # wrappers are put in bin/; originals in resources/
            my $withoutext  = $name-path.subst(/\.[exe|bat]$/, '');
            for '', '-j', '-m' -> $be {
                $.prefix.concat-with("$withoutext$be").IO.spurt:
                    $perl_wrapper.subst('#name#', $name-path.IO.basename, :g).subst('#perl#', "perl6$be").subst('#dist-name#', $dist.meta<name>);
                if $is-win {
                    $.prefix.concat-with("$withoutext$be.bat").IO.spurt:
                        $windows_wrapper.subst('#perl#', "perl6$be", :g);
                }
                else {
                    $.prefix.concat-with("$withoutext$be").IO.chmod(0o755);
                }
            }
        ...
        for %files.kv -> $name-path, $file is copy {
            ...
            my $destination    = $resources-dir.concat-with($id);
            %links{$name-path} = $id;
            my $handle  = $dist.content($file);
            my $content = $handle.open.slurp-rest(:bin,:close);
            $destination.spurt($content);
        ...
        $dist-dir.concat-with($dist-id).spurt: Rakudo::Internals::JSON.to-json(%meta);
...

        {
            ...
            for %provides.kv -> $source-name, $source-meta {
                ...
                my $source = $sources-dir.concat-with($id);
    ...
    method uninstall(Distribution $distribution) {
        ...
        my $sources-dir   = self.prefix.concat-with('sources');
        my $resources-dir = self.prefix.concat-with('resources');
        my $bin-dir       = self.prefix.concat-with('bin');
        my $dist-dir      = self.prefix.concat-with('dist');
        ...
        for %files.kv -> $name-path, $file {
            given $name-path {
                when /^bin\/(.*)/ {
                    ...
                    unlink-if-exists( $bin-dir.concat-with("$0$_") ) for '', '-m', '-j';
                    # original bin scripts are in $resources-dir
                    unlink-if-exists( $resources-dir.concat-with($file) )
                }
                when /^resources\// {
                    unlink-if-exists( $resources-dir.concat-with($file) )
                }
        ...
        # delete sources
        unlink-if-exists( $sources-dir.concat-with($_) ) for %provides.values.flatmap(*.values.map(*.<file>));

        # delete the meta file
        unlink( $dist-dir.concat-with($dist.id) )
    }

    method files($file, :$name!, :$auth, :$ver) {
        my @candi;
        my $prefix = self.prefix;
        my $lookup = $prefix.concat-with('short').concat-with(nqp::sha1($name));
        if $lookup.e {
            my $repo-version = self!repository-version;
            ...
            for @dists.grep({$_<auth> ~~ $auth and $_<ver> ~~ $ver}) -> $dist is copy {
                ...
                with $dist<files>{$file} {
                    my $candi = %$dist;
                    $candi<files>{$file} = self!resources-dir.concat-with($candi<files>{$file});
                    @candi.push: $candi;
                }
            }
        }
    ...
    method !matching-dist(CompUnit::DependencySpecification $spec) {
        if $spec.from eq 'Perl6' {
            my $repo-version = self!repository-version;
            my $lookup = $.prefix.concat-with('short').concat-with(nqp::sha1($spec.short-name));
            if $lookup.e {
                my @dists = (
                        $repo-version < 1
                        ?? $lookup.lines.unique.map({
        ...
        --> CompUnit:D)
    {
        ...
        if $dist-id {
            ...
            my $loader = $.prefix.concat-with('sources').concat-with($source-file-name);
            my $*RESOURCES = Distribution::Resources.new(:repo(self), :$dist-id);
    ...
    method resource($dist-id, $key) {
        my $dist = %!dist-metas{$dist-id} //= Rakudo::Internals::JSON.from-json(self!dist-dir.concat-with($dist-id).slurp);
        # need to strip the leading resources/ on old repositories
        self!resources-dir.concat-with($dist<files>{$key.substr(self!repository-version < 2 ?? 10 !! 0)})
    }
    ...
    method id() {
        ...
        my $dist-dir = $.prefix.concat-with('dist');
