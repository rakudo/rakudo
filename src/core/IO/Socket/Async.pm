my class IO::Socket::Async {
    ...
    method send(IO::Socket::Async:D: Str(Any) $str, :$scheduler = $*SCHEDULER) {
        my $p = Promise.new;
        my $v = $p.vow;
        nqp::asyncwritestr(
            $!VMIO,
...
#say "discarding from $next_seq for $done";
    ...
    method connect(IO::Socket::Async:U: Str(Any) $host, Int(Any) $port,
                   :$scheduler = $*SCHEDULER) {
        my $p = Promise.new;
        my $v = $p.vow;
        nqp::asyncconnect(
    ...
    method listen(IO::Socket::Async:U: Str(Any) $host, Int(Any) $port,
                  :$scheduler = $*SCHEDULER) {
        my $cancellation;
        Supply.on_demand(-> $s {
