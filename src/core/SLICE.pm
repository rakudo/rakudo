    ...
    if nqp::iseq_i(nqp::elems($d),0) && $pair.value {
        ...
        nqp::iseq_s($key,'delete')
          ...
          !! nqp::iseq_s($key,'exists')
            ?? SELF.EXISTS-POS($one)
            !! nqp::iseq_s($key,'kv')
              ?? (SELF.EXISTS-POS($one) ?? ($one, SELF.AT-POS($one)) !! ())
              !! nqp::iseq_s($key,'p')
                ?? (SELF.EXISTS-POS($one) ?? Pair.new($one,SELF.AT-POS($one)) !! ())
                !! nqp::iseq_s($key,'k')
                  ?? (SELF.EXISTS-POS($one) ?? $one !! ())
                  !! nqp::iseq_s($key,'v')
                     ?? (SELF.EXISTS-POS($one) ?? SELF.AT-POS($one) !! ())
                     !! die "Unknown attribute $key for LIST slice with $one";
    }
    ...
    if nqp::iseq_i(nqp::elems($d),0) && $pair.value {
        ...
        nqp::iseq_s($key,'delete')
          ...
          !! nqp::iseq_s($key,'exists')
            ?? SELF.EXISTS-KEY($one)
            !! nqp::iseq_s($key,'kv')
              ?? (SELF.EXISTS-KEY($one) ?? ($one, SELF.AT-KEY($one)) !! ())
              !! nqp::iseq_s($key,'p')
                ?? (SELF.EXISTS-KEY($one) ?? Pair.new($one,SELF.AT-KEY($one)) !! ())
                !! nqp::iseq_s($key,'k')
                  ?? (SELF.EXISTS-KEY($one) ?? $one !! ())
                  !! nqp::iseq_s($key,'v')
                     ?? (SELF.EXISTS-KEY($one) ?? SELF.AT-KEY($one) !! ())
                     !! die "Unknown attribute $key for HASH slice with $one";
    }
