...
my $start     = '#- start of generated ';
my $idpos     = $start.chars;
my $idchars   = 3;
my $end       = '#- end of generated ';
...
for $*IN.lines -> $line {
    ...
    my role @PRE[]array[::T] does Positional[T] is array_type(T) {
        ...
        multi method STORE(@PRE[]array:D: Range:D $range) {
            my @type[] $val = $range.min;
            $val = $val + 1 if $range.excludes-min;
            my @type[] $max = $range.max;
            $max = $max - 1 if $range.excludes-max;
            nqp::setelems(self, $max - $val + 1);

            my int $i;
            while $val <= $max {
                nqp::bindpos_@POST[]self, $i, $val);
                $val = $val + 1;
                $i   = $i   + 1;
            }
            self
