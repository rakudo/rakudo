...
while @lines {
    ...
    # found header
    die "Don't know how to handle $type" unless $type eq "int" | "uin" | "num" | "str";
    ...
    my %mapper =
      ...
      iseq_postfix => $type eq 'uint' ?? 'i' !! $type.substr(0, 1),
      push_postfix => $type eq 'uint' ?? 'i' !! $type.substr(0, 1),
      list_postfix => $type eq 'uint' ?? 'i' !! $type.substr(0, 1),
      type    => $type,
...
#                  nqp::push_#push_postfix#($result,nqp::atpos_#postfix#(self,$i))
#                )
#              )
#            );
#
