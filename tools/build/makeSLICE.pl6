...
sub SLICE_ONE_@TYPE[]\SELF,$one,$pair,%adv) {
    ...
    # simple 1 true adverb
    if nqp::iseq_i(nqp::elems($d),0) && $pair.value {
        my str $key =
          nqp::unbox_s(nqp::getattr(nqp::decont($pair),Pair,'$!key'));
        nqp::iseq_s($key,'delete')
          ?? SELF.@DELETE[]$one)
          !! nqp::iseq_s($key,'exists')
            ?? SELF.@EXISTS[]$one)
            !! nqp::iseq_s($key,'kv')
              ?? (SELF.@EXISTS[]$one) ?? ($one, SELF.@AT[]$one)) !! ())
              !! nqp::iseq_s($key,'p')
                ?? (SELF.@EXISTS[]$one) ?? Pair.new($one,SELF.@AT[]$one)) !! ())
                !! nqp::iseq_s($key,'k')
                  ?? (SELF.@EXISTS[]$one) ?? $one !! ())
                  !! nqp::iseq_s($key,'v')
                     ?? (SELF.@EXISTS[]$one) ?? SELF.@AT[]$one) !! ())
                     !! die "Unknown attribute $key for @TYPE[].chop() slice with $one";
    }
