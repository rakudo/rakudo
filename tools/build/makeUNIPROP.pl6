...
sub get-uni-props is export {
    ...
    for $property-aliases-string.lines -> $line {
        ...
        for @names {
            push @names-alt, $_.trim;
        }
        next if $long-name ~~ / ^ \s* $ /;
        $type = 'B' if ~$type eq 'Binary';
        $type = 'S' if ~$type eq any('Enumerated', 'String', 'Catalog');
        for @names-alt {
            push %hash{$long-name}<names>, $_.trim;
        }
        %hash{$long-name}<type> = ~$type;
    }
    %hash;
}
# This sub just keeps us under 80 chars width when printing
sub print-line (Str $str, Bool :$flush?) {
    my $max-width = 80;
    my $indent = ' ' x 2;
    state Str $full-string = '';
    if $full-string.chars + $str.chars > $max-width or $flush {
        say $indent x 2 ~ $full-string;
        $full-string = '';
    }
    $full-string ~= $str;
}
...
sub create-Str-code {
    ...
    print "  ## The code below was generated by tools/build/makeUNIPROP.pl6\n";
    print "  state \%prop-mappings = nqp::hash(\n";
    for %hash.kv -> $key, $value {
        for $value<names>.flat {
            print-line "'{$_}','$key',";
        }
...
sub test-Unicode {
    ...
    for (0..0x1FFFF) -> $codepoint {
        for %hash.kv -> $key, $value {
            for $value<names>.flat -> $alternate {
